...
def getWordScore(word, n):
    ...
    for k, v in letters.items():
        freqs += v
        score += SCRABBLE_LETTER_VALUES[k] * letters[k]
...
def updateHand(hand, word):
    ...
    freqs = getFrequencyDict(word)

    for k, v in handCopy.items():
        usedLetterFreq = freqs.get(k, 0)
        if usedLetterFreq > 0:
            handCopy[k] -= usedLetterFreq

    updatedHand = { k: v for k, v in handCopy.items() if v > 0 }
...
def isValidWord(word, hand, wordList):
    ...
    if word not in wordList:
        ...
        letterFreqs = getFrequencyDict(word)

    for k, v in letterFreqs.items():
        letter = hand.get(k, 0)
        if letter < v:
            return False
    ...
    while True:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print('"' + word + '" ' + 'earned ' + str(wordScore) + ' points.', end=" ")
